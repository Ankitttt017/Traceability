...
function buildStationPair(machineName, op) {
  ...
  if (!name) return null;
  // Use just the operation number as key to match frontend strategy
  return { key: operation, machineName: name, op: operation, label: `${name} + ${operation}` };
}
...
async function generateIndustrialExcel(res, {
  ...
  rows.forEach((row, index) => {
    ...
    if (!grouped.has(groupKey)) {
      grouped.set(groupKey, {
        ...
        createdAt: row.__pr_first_scan_at || row.firstScanCreatedAt || row.createdAt || row.created_at || "-",
        finalResultAt: row.__pr_final_scan_at || row.finalResultCreatedAt || row.finalResultAt || row.cycleEndAt || row.plc_end_at || row.plcEndAt || "",
        __pr_overall_status: row.__pr_overall_status || null,
        __pr_final_scan_at: row.__pr_final_scan_at || null,
        __pr_first_scan_at: row.__pr_first_scan_at || null,
        cycleStart: row.__pr_first_scan_at || row.cycleStartTime || "-",
    ...
    if (!bucket.finalResultAt && (row.__pr_final_scan_at || row.finalResultCreatedAt || row.finalResultAt || row.cycleEndAt || row.plc_end_at || row.plcEndAt)) {
      bucket.finalResultAt = row.__pr_final_scan_at || row.finalResultCreatedAt || row.finalResultAt || row.cycleEndAt || row.plc_end_at || row.plcEndAt;
    }
    ...
    // Also check nested leakTest arrays (frontend format)
    if (!bucket.leakTestReadings && row.leakTest && Array.isArray(row.leakTest) && row.leakTest.length > 0) {
      bucket.leakTestReadings = row.leakTest;
    }
  ...
  matrixRows.forEach((row, i) => {
    ...
    // Use __pr_overall_status from master metadata if available
    const finalPartStatus = normalizeFinalPartStatus(row.partStatus);
    const masterStatus = row.__pr_overall_status;
    let overall;
    if (masterStatus === 'PASSED' || masterStatus === 'OK') {
      overall = 'PASSED';
    } else if (masterStatus === 'NG' || masterStatus === 'FAILED') {
      overall = 'NG';
    } else if (masterStatus === 'IN_PROGRESS') {
      overall = 'IN_PROGRESS';
    } else {
      overall = operationResults.includes("NG")
        ? "NG"
    ...
    const exportPartName =
      ...
      row.partDieLabel ||
      "-";
    // Use __pr_final_scan_at for final result display if available
    const finalResultSource = row.__pr_final_scan_at || row.finalResultAt;
    const finalResultDisplay = (overall === "PASSED" || overall === "NG") && finalResultSource
      ? formatIndustrialTimestamp(finalResultSource)
      : "-";
    // Use __pr_first_scan_at for first scan display if available
    const firstScanDisplay = row.__pr_first_scan_at || row.cycleStart || row.createdAt;
    const values = [
      i + 1,
      shotNumber,
      row.partSerial,
      ...
      firstScanDisplay !== "-" ? formatIndustrialTimestamp(firstScanDisplay) : "-",
      finalResultDisplay,
      ...stationResults,
      overall,
